      ...
      {features.map((feature, i) => {
        ...
        return (
          <button
            ...
            className={`relative overflow-hidden rounded-2xl border text-left transition-all duration-300 ease-out cursor-pointer group animate-card-enter ${colSpan} ${rowSpan}
              ${isActive
                ? 'bg-white border-yellow shadow-xl shadow-yellow/10'
                : isHovered
                  ? 'bg-white border-yellow/50 shadow-lg shadow-yellow/5 -translate-y-0.5'
                  : 'bg-[#EEF3F0] border-yellow/5 hover:bg-white hover:border-yellow/40 hover:shadow-lg hover:-translate-y-1'
              }`}
...
function LayoutTemplate({ feature, isActive, isHovered, mockupType, mockupSize }) {
  switch (feature.layout) {
    case 'chart-first':
      return <ChartFirstLayout feature={feature} isActive={isActive} isHovered={isHovered} mockupType={mockupType} mockupSize={mockupSize} />
    case 'split':
      return <SplitLayout feature={feature} isActive={isActive} isHovered={isHovered} mockupType={mockupType} mockupSize={mockupSize} />
    case 'kpi-hero':
      return <KpiHeroLayout feature={feature} isActive={isActive} isHovered={isHovered} mockupType={mockupType} mockupSize={mockupSize} />
    default:
      return <DefaultLayout feature={feature} isActive={isActive} isHovered={isHovered} mockupType={mockupType} mockupSize={mockupSize} />
  }
}

function DefaultLayout({ feature, isActive, isHovered, mockupType, mockupSize }) {
  return (
    <>
      <TopBar feature={feature} isActive={isActive} isHovered={isHovered} />
...
function ChartFirstLayout({ feature, isActive, isHovered, mockupType, mockupSize }) {
  return (
    <>
...
function SplitLayout({ feature, isActive, isHovered, mockupType, mockupSize }) {
  return (
    <div className="flex gap-4 h-full">
      <div className="flex flex-col justify-between flex-1 min-w-0">
        <div>
          ...
          <p className="text-[11px] text-noir/50 leading-relaxed mt-1 line-clamp-2">{feature.description}</p>
        </div>
        {feature.kpis && isActive && <KpiRow kpis={feature.kpis} />}
      </div>
...
function KpiHeroLayout({ feature, isActive, isHovered, mockupType, mockupSize }) {
  return (
    <>
        ...
        <div className="flex gap-4 my-2">
          {feature.kpis.map((kpi, ki) => (
            <div key={ki} className="flex-1 bg-gradient-to-br from-yellow/10 to-orange/5 rounded-xl px-3 py-2 border border-yellow/10">
              <span className="text-[9px] text-noir/50 font-medium block">{kpi.label}</span>
...
function TopBar({ feature, isActive, isHovered }) {
  return (
    <div className="flex items-start justify-between gap-3 mb-2">
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div>
          ...
          {feature.size !== 'sm' && (
            <p className="text-[11px] md:text-xs text-noir/50 leading-relaxed mt-1 max-w-md">{feature.description}</p>
          )}
        </div>
      </div>
      <StatusBadge feature={feature} isActive={isActive} />
...
function StatusBadge({ feature, isActive }) {
  return (
    <div className="flex items-center gap-2 shrink-0">
      {feature.status && (
        <span
          ...
          style={{ backgroundColor: `${feature.statusColor}15`, color: feature.statusColor }}
        >
