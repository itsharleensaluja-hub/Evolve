...
function PriceDisplay({ amount, currency, annual, period }) {
  ...
  return (
    <div className="mb-6">
      <div className="flex items-baseline gap-1">
        ...
        <span className="text-sm text-noir/50">
          {period}
        </span>
      </div>
      {annual && (
        <p className="text-xs text-noir/40 mt-1">
          {(amount / 12).toLocaleString(currency.locale, { style: 'currency', currency: currency.code, minimumFractionDigits: 0 })}/month billed annually
        </p>
      )}
    </div>
  )
...
function FeatureCheck({ children }) {
  return (
    <li className="flex items-start gap-3 text-sm">
      ...
      <span className="text-noir/60">{children}</span>
    </li>
  )
}
...
export default function Pricing() {
  ...
  return (
    <section id="pricing" className="py-20 md:py-28 bg-powder" ref={ref}>
      <Container>
        <SectionHeading
          label="Pricing"
        ...
        <div className={`grid md:grid-cols-3 gap-6 transition-all duration-700 delay-200 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          {plans.map((plan, i) => {
            const monthlyPrice = plan.monthly[currency.code]
            const displayPrice = annual ? monthlyPrice * 12 * 0.8 : monthlyPrice
            ...
            return (
              ...
              >
                ...
                <div className="mb-6">
                  <h3 className="font-heading text-lg font-bold text-noir mb-1">
                    {plan.name}
                  </h3>
                  <p className="text-sm text-noir/50 leading-relaxed">
                    {plan.description}
                  </p>
                </div>
                ...
                <div className="mt-auto">
                  <Button
                    variant={plan.popular ? 'primary' : 'secondary-dark'}
                    className="w-full"
                  >
                    {plan.cta}
                  </Button>
